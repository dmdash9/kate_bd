...
var moving_targets: int = 0
var walk_started: bool = false
var greeting: bool = false

var mia_intro_started: bool = false
...
func _ready():
	...
	# Connect the resize signal to handle window resizing
	#get_viewport().connect("size_changed", self._on_size_changed)
	
	$Start.connect("pressed", self.on_start_press)
...
#func _on_size_changed():
	...
	#print("Game Window Size: ", window_size.x, "x", window_size.y)
	#
	#print(dima.position.x, window_size.x)
#
	## Reposition the asset right after the viewport
	#dima.position.x = window_size.x
